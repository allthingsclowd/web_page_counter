...
if [[ "${HOSTNAME}" =~ "leader" ]] || [ "${TRAVIS}" == "true" ]; then
  if [ "${TRAVIS}" == "true" ]; then
    ...
    NOMAD_ADDR=http://${IP}:4646 /usr/local/bin/nomad agent-info 2>/dev/null || {
      create_service nomad "HashiCorp's Nomad Server - A Modern Platform and Cloud Agnostic Scheduler" "/usr/local/bin/nomad agent -log-level=DEBUG -server -bind=0.0.0.0 -data-dir=/usr/local/nomad -bootstrap-expect=1 -config=/etc/nomad.d"
      sudo usermod -a -G consulcerts nomad
