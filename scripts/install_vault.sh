...
EOF
    ...
    curl \
        --location \
        --cacert "/${ROOTCERTPATH}/ssl/certs/vault-ca-chain.pem" \
        --key "/${ROOTCERTPATH}/vault.d/pki/tls/private/vault-cli-key.pem" \
        --cert "/${ROOTCERTPATH}/vault.d/pki/tls/certs/vault-cli.pem" \
        --header "X-Vault-Token: ${VAULT_TOKEN}" \
        --request POST \
        --data @approle.json \
...
install_vault () {
    ...
    if [[ "${HOSTNAME}" =~ "leader" ]] || [ "${TRAVIS}" == "true" ]; then
        ...
        if [ "${TRAVIS}" == "true" ]; then
            ...
            sudo ls -al /${ROOTCERTPATH}/ssl/certs/ /${ROOTCERTPATH}/consul.d/pki/tls/private/ /${ROOTCERTPATH}/consul.d/pki/tls/certs/consul-peer.pem /${ROOTCERTPATH}/vault.d/pki/tls/private/vault-server-key.pem /${ROOTCERTPATH}/vault.d/pki/tls/certs/vault-server.pem
