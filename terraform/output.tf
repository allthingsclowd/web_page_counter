...
locals {
  hashistackdemo = <<AZUREDEMO
      ...
      Nomad Portal  (public)  -   http://${data.azurerm_public_ip.hashicorp_portals}:4646
      Vault Portal  (public)  -   http://${data.azurerm_public_ip.hashicorp_portals}:8200
      Consul Portal (public)  -   https://${data.azurerm_public_ip.hashicorp_portals}:8321
